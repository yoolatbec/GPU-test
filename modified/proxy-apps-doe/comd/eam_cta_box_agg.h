...
void EAM_Force_cta_box_agg_seperate(sim_t sim)
{
...
void EAM_Force_cta_box_agg(sim_t sim)
{
  ...
  while (global_base < nneigh)
  {
    ...
    if (global_neighbor < nneigh) 
    {
      ...
      // compute box center offsets
      real_t dxbox = - sim.r.x[j_particle] + sim.grid.r_box.x[ibox] - sim.grid.r_box.x[jbox];
      real_t dybox = - sim.r.y[j_particle] + sim.grid.r_box.y[ibox] - sim.grid.r_box.y[jbox];
      real_t dzbox = - sim.r.z[j_particle] + sim.grid.r_box.z[ibox] - sim.grid.r_box.z[jbox];
