...
#ifndef ECX_TARGET
#ifdef SEPERATE
    EAM_Force_cta_box_seperate<step><<<grid,block,smem>>>(sim_D);
#else
...
#ifndef ECX_TARGET
#ifdef SEPERATE
    EAM_Force_cta_box_agg_seperate<step><<<grid,block,smem>>>(sim_D);
#else
...
#ifdef SEPERATE
	copy_halos_kernel_seperate<<<grid, block>>>(sim_D);
#else
...
void eam_force_2(sim_t sim_D)
{
	...
	int block = sim_D.max_atoms;
#ifdef SEPERATE
    EAM_Force_2_seperate<<<grid,block>>>(sim_D); 
#else
