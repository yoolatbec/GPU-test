...
//seperate
__global__ 
void EAM_Force_2_seperate(sim_t sim)
{
...
//seperate
__global__ 
void copy_halos_kernel_seperate(sim_t sim)
{
    int iatom = threadIdx.x;
    int ibox = blockIdx.x;
...
void copy_halos_kernel(sim_t sim)
{
    ...
    if (iatom < sim.grid.n_atoms[ibox] && is_halo(sim, ibox)) 
    {
        ...
        // update rhoprime
        real_t tfi = sim.fi[j_particle];
        real_t tx = sim.r.x[j_particle];
        real_t ty = sim.r.y[j_particle];
        real_t tz = sim.r.z[j_particle];

		sim.fi[i_particle] = tfi; 

		// update positions
		sim.r.x[i_particle] = tx;
		sim.r.y[i_particle] = ty;
		sim.r.z[i_particle] = tz;
    }
}
