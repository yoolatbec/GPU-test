...
//seperate
__global__ void mult_gpu_seperate(float *A, float *B, float *C, int wA, int wB){
   // Block index
    int bx = blockIdx.x;
    int by = blockIdx.y;
...
__global__ void mult_gpu(float *A, float *B, float *C, int wA, int wB){
   // Block index
         ...
         a += aStep, b += bStep)
    {
        ...
        // to shared memory; each thread loads
        // one element of each matrix
        float ta = A[a + wA * ty + tx];
        float tb = B[b + wB * ty + tx];
        As[ty][tx] = ta;
        Bs[ty][tx] = tb;
...
#ifdef SEPERATE
    	mult_gpu_seperate<<<grid, threads, 0>>>(A_dev, B_dev, C_dev, MROW, MROW);
#else
