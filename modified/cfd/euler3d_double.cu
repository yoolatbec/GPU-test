...
//mark
//seperate
__global__ void cuda_time_step_seperate(int j, int nelr, double* old_variables, double* variables, double* step_factors, double* fluxes)
{
	const int i = (blockDim.x*blockIdx.x + threadIdx.x);
...
__global__ void cuda_time_step(int j, int nelr, double* old_variables, double* variables, double* step_factors, double* fluxes)
{
	...
	tmp[0] = old_variables[i + VAR_DENSITY*nelr] + factor*fluxes[i + VAR_DENSITY*nelr];
	tmp[1] = old_variables[i + VAR_DENSITY_ENERGY*nelr] + factor*fluxes[i + VAR_DENSITY_ENERGY*nelr];
	tmp[2] = old_variables[i + (VAR_MOMENTUM+0)*nelr] + factor*fluxes[i + (VAR_MOMENTUM+0)*nelr];
	tmp[3] = old_variables[i + (VAR_MOMENTUM+1)*nelr] + factor*fluxes[i + (VAR_MOMENTUM+1)*nelr];
	tmp[4] = old_variables[i + (VAR_MOMENTUM+2)*nelr] + factor*fluxes[i + (VAR_MOMENTUM+2)*nelr];

	variables[i + VAR_DENSITY*nelr] = tmp[0];
	variables[i + VAR_DENSITY_ENERGY*nelr] = tmp[1];
	variables[i + (VAR_MOMENTUM+0)*nelr] = tmp[2];
	variables[i + (VAR_MOMENTUM+1)*nelr] = tmp[3];
	variables[i + (VAR_MOMENTUM+2)*nelr] = tmp[4];
}
...
void time_step(int j, int nelr, double* old_variables, double* variables, double* step_factors, double* fluxes)
{
	dim3 Dg(nelr / block_length), Db(block_length);
#ifdef SEPERATE
	cuda_time_step_seperate<<<Dg,Db>>>(j, nelr, old_variables, variables, step_factors, fluxes);
#else
