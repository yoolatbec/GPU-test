...
void EAM_Force_wa_smem_seperate(sim_t sim)
{
	// 1 warp per 1 atom
...
void EAM_Force_wa_smem(sim_t sim)
{
    ...
    while (neighbor < sim.grid.n_num_neigh[ibox]) 
    {
		...
		if (r2 <= r2cut && r2 > 0.0) 
		{
            ...
            real_t tx = acc_fx[threadIdx.x] + dTmp * dx / r;
            real_t ty = acc_fy[threadIdx.x] + dTmp * dy / r;
            real_t tz = acc_fz[threadIdx.x] + dTmp * dz / r;

            acc_fx[threadIdx.x] = tx;
            acc_fy[threadIdx.x] = ty;
            acc_fz[threadIdx.x] = tz;
		    // acc_fx[threadIdx.x] += dTmp * dx / r;
		    // acc_fy[threadIdx.x] += dTmp * dy / r;
		    // acc_fz[threadIdx.x] += dTmp * dz / r;

		    if (step == 1) {
                real_t te = acc_e[threadIdx.x] + phiTmp;
                real_t trho = acc_rho[threadIdx.x] + rhoTmp;
                acc_e[threadIdx.x] = te;
                acc_rho[threadIdx.x] = trho;
		    //   acc_e[threadIdx.x] += phiTmp;
		    //   acc_rho[threadIdx.x] += rhoTmp;
		    }
		}
	...
	// warp reduction
    if (lane_id < 16) {
        real_t tx = acc_fx[threadIdx.x] + acc_fx[threadIdx.x + 16];
        real_t ty = acc_fy[threadIdx.x] + acc_fy[threadIdx.x + 16];
        real_t tz = acc_fz[threadIdx.x] + acc_fz[threadIdx.x + 16];
        acc_fx[threadIdx.x] = tx;
        acc_fy[threadIdx.x] = ty;
        acc_fz[threadIdx.x] = tz;
	if (step == 1) {
        real_t te = acc_e[threadIdx.x] + acc_e[threadIdx.x + 16];
        real_t trho = acc_rho[threadIdx.x] + acc_rho[threadIdx.x + 16];
          acc_e[threadIdx.x] = te;
          acc_rho[threadIdx.x] = trho;
 	}
	}
	if (lane_id < 8) {
        real_t tx = acc_fx[threadIdx.x] + acc_fx[threadIdx.x + 8];
        real_t ty = acc_fy[threadIdx.x] + acc_fy[threadIdx.x + 8];
        real_t tz = acc_fz[threadIdx.x] + acc_fz[threadIdx.x + 8];
        acc_fx[threadIdx.x] = tx;
        acc_fy[threadIdx.x] = ty;
        acc_fz[threadIdx.x] = tz;
	if (step == 1) {
        real_t te = acc_e[threadIdx.x] + acc_e[threadIdx.x + 8];
        real_t trho = acc_rho[threadIdx.x] + acc_rho[threadIdx.x + 8];
        acc_e[threadIdx.x] = te;
        acc_rho[threadIdx.x] = trho;
 	}
	}
	if (lane_id < 4) {
        real_t tx = acc_fx[threadIdx.x] + acc_fx[threadIdx.x + 4];
        real_t ty = acc_fy[threadIdx.x] + acc_fy[threadIdx.x + 4];
        real_t tz = acc_fz[threadIdx.x] + acc_fz[threadIdx.x + 4];
        acc_fx[threadIdx.x] = tx;
        acc_fy[threadIdx.x] = ty;
        acc_fz[threadIdx.x] = tz;
	if (step == 1) {
        real_t te = acc_e[threadIdx.x] + acc_e[threadIdx.x + 4];
        real_t trho = acc_rho[threadIdx.x] + acc_rho[threadIdx.x + 4];
        acc_e[threadIdx.x] = te;
        acc_rho[threadIdx.x] = trho;
 	}
	}
	if (lane_id < 2) {
        real_t tx = acc_fx[threadIdx.x] + acc_fx[threadIdx.x + 2];
        real_t ty = acc_fy[threadIdx.x] + acc_fy[threadIdx.x + 2];
        real_t tz = acc_fz[threadIdx.x] + acc_fz[threadIdx.x + 2];
        acc_fx[threadIdx.x] = tx;
        acc_fy[threadIdx.x] = ty;
        acc_fz[threadIdx.x] = tz;
	if (step == 1) {
        real_t te = acc_e[threadIdx.x] + acc_e[threadIdx.x + 2];
        real_t trho = acc_rho[threadIdx.x] + acc_rho[threadIdx.x + 2];
        acc_e[threadIdx.x] = te;
        acc_rho[threadIdx.x] = trho;
 	}
	}
	if (lane_id < 1) {
        real_t tx = acc_fx[threadIdx.x] + acc_fx[threadIdx.x + 1];
        real_t ty = acc_fy[threadIdx.x] + acc_fy[threadIdx.x + 1];
        real_t tz = acc_fz[threadIdx.x] + acc_fz[threadIdx.x + 1];
        acc_fx[threadIdx.x] = tx;
        acc_fy[threadIdx.x] = ty;
        acc_fz[threadIdx.x] = tz;
	if (step == 1) {
        real_t te = acc_e[threadIdx.x] + acc_e[threadIdx.x + 1];
        real_t trho = acc_rho[threadIdx.x] + acc_rho[threadIdx.x + 1];
        acc_e[threadIdx.x] = te;
        acc_rho[threadIdx.x] = trho;
 	}
	}
    ...
    if (lane_id == 0) 
    {
        real_t tx = acc_fx[warp_id * 32];
        real_t ty = acc_fy[warp_id * 32];
        real_t tz = acc_fz[warp_id * 32];
