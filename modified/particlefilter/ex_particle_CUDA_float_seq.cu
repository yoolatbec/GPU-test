 ...
 *****************************/
 //mark
 //seperate
__global__ void find_index_kernel_seperate(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, double * weights, int Nparticles) {
 ...
 *****************************/
//seperate
__global__ void likelihood_kernel_seperate(double * arrayX, double * arrayY, double * xj, double * yj, double * CDF, int * ind, int * objxy, double * likelihood, unsigned char * I, double * u, double * weights, int Nparticles, int countOnes, int max_size, int k, int IszY, int Nfr, int *seed, double* partial_sums) {
    int block_id = blockIdx.x;
    int i = blockDim.x * block_id + threadIdx.x;
    int y;
...
__global__ void likelihood_kernel(double * arrayX, double * arrayY, double * xj, double * yj, double * CDF, int * ind, int * objxy, double * likelihood, unsigned char * I, double * u, double * weights, int Nparticles, int countOnes, int max_size, int k, int IszY, int Nfr, int *seed, double* partial_sums) {
    ...
    if (i < Nparticles) {
        double tx = xj[i];
        double ty = yj[i];
        double tax = xj[i] + 1.0 + 5.0 * d_randn(seed, i);
        double tay = yj[i] - 2.0 + 2.0 * d_randn(seed, i);
    ...
    if (i < Nparticles) {
        ...
        double tl = likelihood[i] / countOnes;
        double tw = weights[i] * exp(likelihood[i] / countOnes);
        
        likelihood[i] = tl;
        weights[i] = tw; //Donnie Newell - added the missing exponential function call
...
void particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    for (k = 1; k < Nfr; k++) {
...
#ifdef SEPERATE
        likelihood_kernel_seperate << < num_blocks, threads_per_block >> > (arrayX_GPU, arrayY_GPU, xj_GPU, yj_GPU, CDF_GPU, ind_GPU, objxy_GPU, likelihood_GPU, I_GPU, u_GPU, weights_GPU, Nparticles, countOnes, max_size, k, IszY, Nfr, seed_GPU, partial_sums);
        ...
        find_index_kernel_seperate << < num_blocks, threads_per_block >> > (arrayX_GPU, arrayY_GPU, CDF_GPU, u_GPU, xj_GPU, yj_GPU, weights_GPU, Nparticles);
#else
