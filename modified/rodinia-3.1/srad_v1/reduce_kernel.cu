...
//mark
//seperate
__global__ void reduce_seperate(	long d_Ne,											// number of elements in array
...
__global__ void reduce(	long d_Ne,											// number of elements in array
	...
	// copy data to shared memory
	if(ei<d_no){															// do only for the number of elements, omit extra threads
        float ts = d_sums[ei* d_mul];
        float ts2 = d_sums2[ei * d_mul];
	...
	if(nf == NUMBER_THREADS){
		...
		for(i=2; i<=NUMBER_THREADS; i=2*i){
			// sum of elements
			if((tx+1) % i == 0){		
                float ts = d_psum[tx] + d_psum[tx - i / 2];
                float ts2 = d_psum2[tx] + d_psum2[tx - i / 2];									// every ith
				d_psum[tx] = ts;
				d_psum2[tx] = ts2;
			}
			// synchronization
		...
		if(tx==(NUMBER_THREADS-1)){			
            float ts = d_psum[tx];
            float ts2 = d_psum2[tx];								// block result stored in global memory
			d_sums[bx*d_mul*NUMBER_THREADS] = ts;
			d_sums2[bx*d_mul*NUMBER_THREADS] = ts2;
		}
	}
	// reduction of sums if last block is not full (common case)
	else{ 
		...
		// for not full block (last block)
			...
			for(i=2; i<=df; i=2*i){											//
				...
				if((tx+1) % i == 0 && tx<df){								// every ith
                    float ts = d_psum[tx] + d_psum[tx - i / 2];
                    float ts2 = d_psum2[tx] + d_psum2[tx - i / 2];
					d_psum[tx] = ts;
					d_psum2[tx] = ts2;
				}
				// synchronization (all threads)
			...
			if(tx==(df-1)){										//
				// compute the remainder and final summation by last busy thread
				for(i=(bx*NUMBER_THREADS)+df; i<(bx*NUMBER_THREADS)+nf; i++){						//
                    float ts = d_psum[tx] + d_sums[i];
                    float ts2 = d_psum2[tx] + d_sums2[i];
					d_psum[tx] = ts;
					d_psum2[tx] = ts2;
				}
				// final sumation by last thread in every block
