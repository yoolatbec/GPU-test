...
//seperate
__global__ void tex_bm_kernel_seperate( float* out, unsigned size)
{
...
__global__ void tex_bm_kernel( float* out, unsigned size)
{
	...
	if(tid < size){
		for(unsigned i=0; i<ITERATIONS; ++i){
            ...
            tmp[0] = tex1Dfetch(texmem1,tid);
			tmp[1] = ConstArray1[(tid+i)%THREADS_PER_BLOCK];
			tmp[2] =  I1[(tid+i)%THREADS_PER_BLOCK];
			tmp[3] = tex1Dfetch(texmem4,tid)*I2[tid%THREADS_PER_BLOCK];
			tmp[4] = ConstArray2[(tid+i)%THREADS_PER_BLOCK]+I8[(tid+i)%THREADS_PER_BLOCK];
			tmp[5] = tex1Dfetch(texmem6,tid)/I6[(tid+i)%THREADS_PER_BLOCK];
			tmp[6] = I3[tid%THREADS_PER_BLOCK]+I4[(tid+i)%THREADS_PER_BLOCK];
			tmp[7] = exp(tex1Dfetch(texmem8,tid));
			tmp[8] = sqrt(abs(ConstArray3[(tid+i)%THREADS_PER_BLOCK]));
...
int main( int argc, char** argv) 
{
    ...
    for(int r = 0; r < TEST_ROUND; r++){
...
#ifdef SEPERATE
        tex_bm_kernel_seperate<<< grid, threads, 0 >>>(device_out, texmem_size);
#else
        ...
        printf("elapsed time: %fms, min time: %fms\n", elapsed_time, min_time);
    }
